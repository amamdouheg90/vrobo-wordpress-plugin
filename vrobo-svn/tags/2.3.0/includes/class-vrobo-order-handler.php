...
class Vrobo_Order_Handler {
    ...
    private function init_hooks() {
        // Hook into order creation using the correct hooks for different checkout types
        // Classic checkout - fires after order is fully processed and saved
        add_action('woocommerce_checkout_order_processed', array($this, 'handle_checkout_order_processed'), 10, 3);
        
        // Block checkout - fires after order is processed via Store API
        add_action('woocommerce_store_api_checkout_order_processed', array($this, 'handle_new_order_hpos'), 10, 1);
        
        // Fallback for admin-created orders and other sources
    ...
    /**
     * Handle checkout order processed (Classic checkout - recommended hook)
     */
    ...
    /**
     * Handle new order creation (Fallback for admin orders and other sources)
     */
    public function handle_new_order($order_id, $order = null) {
        ...
        // Skip if this is during checkout (will be handled by woocommerce_checkout_order_processed)
        ...
        // Store order in database (don't exit if it fails, still send webhook)
    ...
    private function store_order_in_db($order_id, $order) {
        ...
        if ($result !== false) {
            ...
            error_log('Vrobo: Failed to insert order ' . $order_id . ' into database. Error: ' . $wpdb->last_error);
        }
    ...
    private function send_order_to_webhook($order_id, $order) {
        ...
        // If update failed (likely because order doesn't exist), try to insert it
        if ($update_result === 0) {
            $customer_email = $order->get_billing_email();
            $customer_name = trim($order->get_billing_first_name() . ' ' . $order->get_billing_last_name());
            $order_status = $order->get_status();
